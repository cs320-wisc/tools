...
class Grader(Database):
    ...
    def run_test_in_docker(
        ...
        try:
            ...
            result = {"score": 0, "error": str(e), "logs": logs[:50_000].split("\n")}
    ...
    def run_grader(self):
        ...
        for project_id in self.projects:
            ...
            for s3path in sorted(submissions):
                ...
                try:
                    # Setup environment
                    code_dir, submission_fname = self.fetch_submission(
                        s3path, filename=self.conf.FORCE_FILENAME
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Auto-grader for CS320", epilog=extra_help
    )

    # Note: default=argparse.SUPPRESS removes the kwargs from the parsed args if not set.
    parser.add_argument(
        "projects", type=str, nargs="+", help="id(s) of project to run autograder on."
    )
    parser.add_argument(
        "netid",
        type=str,
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        dest="grader_config_path",
        default="./graderconfig.json",
