...
def main():
    ...
    submitters = [email.replace("*at*", "@") for email in os.listdir(f"s3/{COURSE}/projects/{project}")]
    if not os.path.exists("roster.json"):
        print("Need roster.  Run:")
        print(f"aws s3 cp s3://caraza-harter-cs301/{COURSE}/roster.json roster.json")
        return
